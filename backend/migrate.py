...
def migrate_to_postgresql():
    ...
    # Use autocommit to prevent a single failing statement from aborting the whole run.
    # If the returned connection is the PG adapter wrapper, set autocommit on
    # the underlying psycopg2 connection object for real autocommit behavior.
    try:
        if hasattr(conn, '_conn'):
            try:
                ...
                try:
                    conn._conn.set_session(autocommit=True)
                except Exception:
                    pass
        else:
    ...
    try:
        ...
        """)
        # Core application tables (expanded)
        cur.execute("""
        ...
        # Ensure legacy columns expected by the app exist and populate `name` from `full_name` if missing
        try:
            cur.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS name VARCHAR(255)")
            cur.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS role VARCHAR(100) DEFAULT ''")
            cur.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS center_id INTEGER DEFAULT 0")
            # Ensure is_active is an integer to match legacy queries using COALESCE(is_active,1)=1
            try:
                cur.execute("ALTER TABLE users ALTER COLUMN is_active TYPE INTEGER USING (CASE WHEN is_active THEN 1 ELSE 0 END)")
            except Exception:
                pass
            try:
                cur.execute("UPDATE users SET name=full_name WHERE (name IS NULL OR name='') AND (full_name IS NOT NULL AND full_name<>'')")
            except Exception:
                pass
        except Exception:
            pass
        ...
        # Ensure common `recipes` columns exist (idempotent ALTERs)
        for col, defn in [
            ("is_subrecipe", "INTEGER NOT NULL DEFAULT 0"),
            ...
            ("yield_final_qty", "NUMERIC NOT NULL DEFAULT 0"),
            ("yield_final_unit", "VARCHAR(50) NOT NULL DEFAULT 'g'"),
            ("is_active", "INTEGER NOT NULL DEFAULT 1"),
            ("recipe_photo_path", "VARCHAR(512)"),
            ("prep_time_min", "NUMERIC NOT NULL DEFAULT 0"),
            ("cook_time_min", "NUMERIC NOT NULL DEFAULT 0"),
            ("rest_time_min", "NUMERIC NOT NULL DEFAULT 0"),
            ("labor_people", "NUMERIC NOT NULL DEFAULT 0"),
            ("labor_hourly_cost", "NUMERIC NOT NULL DEFAULT 0"),
            ("indirect_sales_base", "NUMERIC NOT NULL DEFAULT 0"),
            ("indirect_rent_amount", "NUMERIC NOT NULL DEFAULT 0"),
            ("indirect_rent_tax_mode", "TEXT NOT NULL DEFAULT 'ex_vat'"),
            ("indirect_services_amount", "NUMERIC NOT NULL DEFAULT 0"),
            ("indirect_services_tax_mode", "TEXT NOT NULL DEFAULT 'ex_vat'"),
            ("indirect_admin_amount", "NUMERIC NOT NULL DEFAULT 0"),
            ("indirect_admin_tax_mode", "TEXT NOT NULL DEFAULT 'ex_vat'"),
            ("indirect_marketing_amount", "NUMERIC NOT NULL DEFAULT 0"),
            ("indirect_marketing_tax_mode", "TEXT NOT NULL DEFAULT 'ex_vat'"),
            ("indirect_other_amount", "NUMERIC NOT NULL DEFAULT 0"),
            ("indirect_other_tax_mode", "TEXT NOT NULL DEFAULT 'ex_vat'"),
            ("salary_cost_amount", "NUMERIC NOT NULL DEFAULT 0"),
            ("created_at", "TIMESTAMP DEFAULT CURRENT_TIMESTAMP"),
            ("updated_at", "TIMESTAMP DEFAULT CURRENT_TIMESTAMP"),
        ]:
            try:
        ...
        # Extra idempotent safeguards for `recipes` columns referenced around the app
        # (some deployments created an older `recipes` table without these fields)
        try:
            cur.execute("ALTER TABLE recipes ADD COLUMN IF NOT EXISTS is_producible INTEGER NOT NULL DEFAULT 0")
            cur.execute("ALTER TABLE recipes ADD COLUMN IF NOT EXISTS produced_item_id INTEGER NOT NULL DEFAULT 0")
            cur.execute("ALTER TABLE recipes ADD COLUMN IF NOT EXISTS yield_portions NUMERIC NOT NULL DEFAULT 1")
            cur.execute("ALTER TABLE recipes ADD COLUMN IF NOT EXISTS yield_final_qty NUMERIC NOT NULL DEFAULT 0")
            cur.execute("ALTER TABLE recipes ADD COLUMN IF NOT EXISTS yield_final_unit VARCHAR(50) NOT NULL DEFAULT 'g'")
            cur.execute("ALTER TABLE recipes ADD COLUMN IF NOT EXISTS is_active INTEGER NOT NULL DEFAULT 1")
            cur.execute("ALTER TABLE recipes ADD COLUMN IF NOT EXISTS recipe_photo_path VARCHAR(512)")
            cur.execute("ALTER TABLE recipes ADD COLUMN IF NOT EXISTS prep_time_min NUMERIC NOT NULL DEFAULT 0")
            cur.execute("ALTER TABLE recipes ADD COLUMN IF NOT EXISTS cook_time_min NUMERIC NOT NULL DEFAULT 0")
            cur.execute("ALTER TABLE recipes ADD COLUMN IF NOT EXISTS rest_time_min NUMERIC NOT NULL DEFAULT 0")
            cur.execute("ALTER TABLE recipes ADD COLUMN IF NOT EXISTS labor_people NUMERIC NOT NULL DEFAULT 0")
            cur.execute("ALTER TABLE recipes ADD COLUMN IF NOT EXISTS labor_hourly_cost NUMERIC NOT NULL DEFAULT 0")
            cur.execute("ALTER TABLE recipes ADD COLUMN IF NOT EXISTS indirect_sales_base NUMERIC NOT NULL DEFAULT 0")
            cur.execute("ALTER TABLE recipes ADD COLUMN IF NOT EXISTS salary_cost_amount NUMERIC NOT NULL DEFAULT 0")
        except Exception:
            pass

        # Ensure POS/TPV date-like columns are TEXT so SQLite-style text operations work
        try:
            cur.execute("ALTER TABLE pos_sales_daily ALTER COLUMN sale_date TYPE TEXT USING sale_date::text")
            cur.execute("ALTER TABLE pos_sales_item_daily ALTER COLUMN sale_date TYPE TEXT USING sale_date::text")
            cur.execute("ALTER TABLE pos_sales_modifier_daily ALTER COLUMN sale_date TYPE TEXT USING sale_date::text")
            cur.execute("ALTER TABLE pos_modifier_consumption_audit ALTER COLUMN sale_date TYPE TEXT USING sale_date::text")
            cur.execute("ALTER TABLE pos_modifier_review_queue ALTER COLUMN sale_date TYPE TEXT USING sale_date::text")
        except Exception:
            pass

        # TPV dates
        try:
            cur.execute("ALTER TABLE tpv_sales ALTER COLUMN sale_datetime TYPE TEXT USING sale_datetime::text")
            cur.execute("ALTER TABLE tpv_sales ALTER COLUMN business_date TYPE TEXT USING business_date::text")
        except Exception:
            pass

        # Convert specific created_at/validated_at/movement_datetime fields used with substr()/text ops
        try:
            # bar_stock_movements: movement_datetime and created_at are used via substr(...)
            cur.execute("ALTER TABLE bar_stock_movements ALTER COLUMN movement_datetime TYPE TEXT USING movement_datetime::text")
            cur.execute("ALTER TABLE bar_stock_movements ALTER COLUMN created_at TYPE TEXT USING created_at::text")
        except Exception:
            pass

        try:
            # receipts: doc_date is TEXT but validated_at/created_at may be TIMESTAMP in older schemas
            cur.execute("ALTER TABLE receipts ADD COLUMN IF NOT EXISTS validated_at TEXT")
            cur.execute("ALTER TABLE receipts ALTER COLUMN validated_at TYPE TEXT USING validated_at::text")
            cur.execute("ALTER TABLE receipts ALTER COLUMN created_at TYPE TEXT USING created_at::text")
        except Exception:
            pass

        try:
            # waste_records: ensure confirmed_at exists as TEXT and created_at is text for substr(...)
        ...
        # Additional core tables and accounting/supplier documents
        cur.execute("""
        ...
        # Ensure items and inventory columns present (safe idempotent ALTERs)
        try:
            cur.execute("ALTER TABLE items ADD COLUMN IF NOT EXISTS unit TEXT NOT NULL DEFAULT ''")
            cur.execute("ALTER TABLE items ADD COLUMN IF NOT EXISTS min_qty DOUBLE PRECISION NOT NULL DEFAULT 0")
            cur.execute("ALTER TABLE items ADD COLUMN IF NOT EXISTS max_qty DOUBLE PRECISION NOT NULL DEFAULT 0")
            cur.execute("ALTER TABLE items ADD COLUMN IF NOT EXISTS current_price DOUBLE PRECISION NOT NULL DEFAULT 0")
            cur.execute("ALTER TABLE items ADD COLUMN IF NOT EXISTS waste_default_pct DOUBLE PRECISION NOT NULL DEFAULT 0")
            cur.execute("ALTER TABLE items ADD COLUMN IF NOT EXISTS stock_area TEXT NOT NULL DEFAULT ''")
        except Exception:
            pass

        try:
            cur.execute("ALTER TABLE inventory_sessions ADD COLUMN IF NOT EXISTS warehouse_id INTEGER")
            cur.execute("ALTER TABLE inventory_sessions ADD COLUMN IF NOT EXISTS session_type TEXT NOT NULL DEFAULT 'MIXTO'")
            cur.execute("ALTER TABLE inventory_sessions ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'DRAFT'")
            cur.execute("ALTER TABLE inventory_sessions ADD COLUMN IF NOT EXISTS note TEXT")
        except Exception:
            pass
        ...
        try:
            ...
            cur.execute("ALTER TABLE inventory_counts ADD COLUMN IF NOT EXISTS family_key TEXT NOT NULL DEFAULT ''")
            cur.execute("ALTER TABLE inventory_counts ADD COLUMN IF NOT EXISTS warehouse_id INTEGER NOT NULL DEFAULT 0")
            cur.execute("ALTER TABLE inventory_counts ADD COLUMN IF NOT EXISTS theoretical_qty DOUBLE PRECISION NOT NULL DEFAULT 0")
            cur.execute("ALTER TABLE inventory_counts ADD COLUMN IF NOT EXISTS physical_qty DOUBLE PRECISION NOT NULL DEFAULT 0")
            cur.execute("ALTER TABLE inventory_counts ADD COLUMN IF NOT EXISTS count_unit TEXT NOT NULL DEFAULT 'ud'")
            cur.execute("ALTER TABLE inventory_counts ADD COLUMN IF NOT EXISTS is_checked BOOLEAN NOT NULL DEFAULT FALSE")
            cur.execute("ALTER TABLE inventory_counts ADD COLUMN IF NOT EXISTS unit_cost_snapshot DOUBLE PRECISION NOT NULL DEFAULT 0")
        except Exception:
            pass

        # Add supplier/accounting indexes if needed
        try:
            cur.execute("CREATE INDEX IF NOT EXISTS idx_supplier_documents_supplier ON supplier_documents(supplier_id,created_at)")
        except Exception:
            pass
